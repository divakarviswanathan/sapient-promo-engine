...
@Service
public class CashBackOffer implements IOffer {
	...
	public PEOrderPromotion applyOffer(OrderDTO order, PEPromotion pePromo) {
		if(pePromo.isAtItemLevel()) {
			return offerHelper.applyOfferAtItemLevel(order, pePromo, PromotionConstants.CASH_BACK_STR);
		} else {
			return offerHelper.applyOfferAtOrderLevel(order, pePromo, PromotionConstants.CASH_BACK_STR);
		}
	}
