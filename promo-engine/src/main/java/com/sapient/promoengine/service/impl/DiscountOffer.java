...
public class DiscountOffer implements IOffer {
	...
	public PEOrderPromotion applyOffer(OrderDTO order, PEPromotion pePromo) {
		if(pePromo.isAtItemLevel()) {
			return offerHelper.applyOfferAtItemLevel(order, pePromo, PromotionConstants.DISCOUNT_STR);
		} else {
			return offerHelper.applyOfferAtOrderLevel(order, pePromo, PromotionConstants.DISCOUNT_STR);
		}
	}
